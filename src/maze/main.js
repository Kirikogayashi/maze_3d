...
function init() {
	...
	const plane = getPlane(fieldSize);
...
function getParticles() {
	const particlesGeo = new THREE.SphereGeometry();
	const particlesMas = new THREE.PointsMaterial({
		color: "#ffffff",
		size: 1,
		transparent: true,
	});
	...
	for ( let i = 0; i < particlesCount; i++) {
		const posX = (Math.random()) * particlesDistance;
		const posY = (Math.random()) * particlesDistance;
		const posZ = (Math.random()) * particlesDistance;
		const particle = new THREE.Vector3(posX, posY, posZ);

		particlesGeo.vertices.push(particle);
	}

	const particleSystem = new THREE.Points(particlesGeo, particlesMas);
...
function getMob(){
	const pointLight = getPointLight(1, "rgb(255, 0, 0)");
	pointLight.name = "mob";
